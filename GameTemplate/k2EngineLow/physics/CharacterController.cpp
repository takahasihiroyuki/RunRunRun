...
namespace nsK2EngineLow {
	...
	const void CharacterController::Set_r(float In_r)
	{
		m_r = In_r;
	}
	//増やした
	const Vector3& CharacterController::Execute(float theta, Vector3& moveSpeed, float deltaTime)
	{
		if (moveSpeed.y > 0.0f) {
			//吹っ飛び中にする。
			m_isJump = true;
		...
		//次の移動先となる座標を計算する。
		Vector3 nextPosition = { sin(theta) * -(m_r + 1000), /*moveSpeed.y*/0.0f, cos(theta) * -(m_r + 1000) };
		////速度からこのフレームでの移動量を求める。オイラー積分。
		Vector3 addPos = moveSpeed;
	
		addPos.Scale(deltaTime);
		nextPosition.Add(addPos);
		Vector3 originalXZDir = /*nextPosition*/addPos;
		originalXZDir.y = 0.0f;
		originalXZDir.Normalize();
		//XZ平面での衝突検出と衝突解決を行う。
		{
